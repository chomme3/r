...
summary(twfe)
#Note: We get the same coefficient from the Lecture 3 slides (slide 27)
...
#Note: This allows us to see the vector of betas which go into calculating B_twfe and the corresponding weights.
#Note:The problematic 2x2 DiD estimate is 2 Later vs Earlier Treated. But the weight is pretty small here so there's probably not much bias.
#---------------------------------------------------------------------------------------
...
#Note: We don't have the problem of negative weights in this particular application.
...
min_year <- min(acafs$rel_year)
# Formula we will use
formula_twfe_es <- as.formula(glue::glue("dins ~ i(rel_year, drop=c(-1, {min_year})) | stfips2 + year2"))
...
atts_never <- did::att_gt(yname = "dins", # name of the LHS variable
                          ...
                          xformla = NULL, #if we had covariates, we could add "~ x1 + x2 + x3,"
                          weightsname = "W",
                          ...
                          panel = TRUE) # whether the data is panel or repeated cross-sectional; if panel is unbalanced, add option "allow_unbalanceD_panel = TRUE" below this
...
#Note: This makes the event study plot using all the att_gts.

agg_effects_es_never <- aggte(atts_never, type = "dynamic", min_e = -5, max_e = 5)
#Note: aggte is a "glue" function which aggregates all the att_gts together to create an event study.
