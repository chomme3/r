...
#      Codes based on Callaway and Sant'Anna (2020)
#---------------------------------------------------
#-----------------------------------------------------------------------------
# Load packages
#-----------------------------------------------------------------------------
...
# Formula we will use
formula_twfe_es_unc <- as.formula(glue::glue("lemp ~ i(rel_year, drop=c(-1, {min_year})) | first.treat + year"))
...
# Formula we will use
formula_twfe_es_cond <- as.formula(glue::glue("lemp ~ i(rel_year, drop=c(-1, {min_year})) +
